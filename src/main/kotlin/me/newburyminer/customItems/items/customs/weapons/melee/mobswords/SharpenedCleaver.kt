...
class SharpenedCleaver: CustomItemDefinition {

    override val custom: CustomItem = CustomItem.SHARPENED_CLEAVER

    private val material = Material.DIAMOND_SWORD
    private val color = arrayOf(237, 183, 33)
    private val name = Utils.text("Sharpened Cleaver", color)
    private val lore = Utils.loreBlockToList(
        Utils.text("Piercing weapon type, sweeps to hit many mobs: ", Utils.GRAY),
        Utils.text("Hitbox Margins: +0.4 blocks, Range: 3.75 blocks.", Utils.GRAY),
        Utils.text("Deals 17% increased damage to Arid Lands mobs.", Utils.GRAY),
    )
    ...
    override val item: ItemStack = CustomItemBuilder(material, custom)
        ...
        .setData(
            ...
            AttackRange.attackRange().hitboxMargin(0.4F).minReach(0.0F).maxReach(3.75F).build()
        )
        .setLore(lore)
        .apply {
            this.addExtraSlayer(0.17)
        }
        .build()
