...
class Polygon(val y: Double, private var points: List<Location>): Shape() {
    ...
    override fun contains(loc: Location): Boolean {
        ...
        for (i in 0..<(points.size)) {
            ...
            if (loc.z < min(p1.z, p2.z) || loc.z > max(p1.z, p2.z) || loc.x > max(p1.x, p2.x)) continue
            val intersectX = (p2.x - p1.x) / (p2.z - p1.z) * (loc.z - p1.z) + p1.x
            if (loc.x > intersectX) continue
