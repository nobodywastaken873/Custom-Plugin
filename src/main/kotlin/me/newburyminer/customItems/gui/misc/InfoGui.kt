...
class InfoGui: CustomGui() {
    ...
    init {
        ...
        val gravesInfo = ItemStack(Material.CHEST)
            ...
            .loreBlock(Utils.text("Upon dying, a grave will be places where you die with all of your items. If another player killed you, they will be able to loot one (or two if they have a certain upgrade) random gear item(s) from your grave. Otherwise, noone else will be able to access your grave. You can right click on your grave to open it, or sneak right click to claim all items in it. All of your graves can be viewed in /graves, where you can see their location. Additionally, by clicking on a grave in that menu, you can teleport to it. You cannot teleport to graves for 5 minutes after dying, and each teleport will cost 1 diamond more than the last.", arrayOf(191, 191, 187)))
        ...
        val survivalTimeInfo = ItemStack(Material.SHIELD)
            .name(Utils.text("Survival Time", arrayOf(111, 199, 119)))
            .loreBlock(Utils.text("Not dying for extended periods of time now grants you buffs. Starting at 4 hours survived, you will gain mining efficiency and double chest loot chance, scaling up to a max of +10 mining efficiency and +40% double loot chance at 20 hours survived. View your current time with /survivaltime.", arrayOf(191, 191, 187)))

        val kothInfo = ItemStack(Material.QUARTZ_BLOCK)
            .name(Utils.text("KOTH Info", arrayOf(57, 72, 237)))
            .loreBlock(Utils.text("Every day between 2-2:30, if at least 5 active players are online, a King of the Hill event will occur at a random location in the world for 30 minutes. You will be able to view your and other players' times on the hill in the tab menu. The 1st and 2nd place players will receive rewards, which can be found in /kothloot.", arrayOf(191, 191, 187)))
        ...
        val customItemInfo = ItemStack(Material.NETHERITE_PICKAXE)
            ...
            .loreBlock(Utils.text("A lot of custom items have been added, you can view all of them in /recipe. Once you have gathered all of the materials, you can use /craft to open a custom crafting menu where you can craft the item.", arrayOf(191, 191, 187)))

        val aridLandsInfo = ItemStack(Material.RED_SAND)
            .name(Utils.text("Arid Lands", arrayOf(247, 202, 87)))
            .loreBlock(Utils.text("A new dimension has been added, the Arid Lands, with custom ores, structures, mobs, and more. The recipes and loot tables for the dimension are nearly complete and will be released in the next couple of days. You will be able to enter through a portal at 0, 100, 0, and, similarly to the end, it will open in 1.5-2 weeks. More info available later in /aridinfo.", arrayOf(191, 191, 187)))

        val endInfo = ItemStack(Material.END_PORTAL_FRAME)
            .name(Utils.text("End", arrayOf(247, 207, 2)))
            .loreBlock(Utils.text("The End is currently locked, and will open about 1-1.5 weeks after the start of the server.", arrayOf(191, 191, 187)))

        val worldInfo = ItemStack(Material.BARRIER)
            .name(Utils.text("Worlds", arrayOf(247, 207, 2)))
            .loreBlock(Utils.text("The worldborder is currently at 20k x 20k in the overworld and nether. In a couple weeks, the End and Arid Lands dimension will open with a border of 20k x 20k as well.", arrayOf(191, 191, 187)))

        inv.setItem(19, combatInfo)
        inv.setItem(20, afkInfo)
        inv.setItem(21, trackingInfo)
        inv.setItem(22, gravesInfo)
        inv.setItem(23, durabilityInfo)
        inv.setItem(24, survivalTimeInfo)
        inv.setItem(25, kothInfo)
        inv.setItem(28, trimInfo)
        inv.setItem(29, griefingInfo)
        inv.setItem(30, customItemInfo)
        inv.setItem(32, aridLandsInfo)
        inv.setItem(33, endInfo)
        inv.setItem(34, worldInfo)
