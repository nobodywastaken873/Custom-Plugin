...
class HardWardenSpawner: CustomItemDefinition {

    override val custom: CustomItem = CustomItem.HARD_WARDEN_SPAWNER

    private val material = Material.POPPED_CHORUS_FRUIT
    private val color = arrayOf(1, 69, 92)
    private val name = text("Hard Warden Spawner", color)
    private val lore = Utils.loreBlockToList(
        text("Right click to consume this item and begin the hard difficulty custom warden boss. It will teleport players within 10 blocks of you as well. You cannot use this while being tracked, while in combat, or if someone else is fighting the boss already.", Utils.GRAY)
    )
    ...
    init {
        ...
        {e ->
            ...
            val spawnedBoss = BossManager.spawnBoss(boss, BossDifficulty.HARD, e.player, players.toList())
